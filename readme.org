...
* Use
  ...
  ~stumpbuffer-show-frames-p~ can be set to ~nil~ to hide frames from
  the list.
...
** Stumpwm
   ...
   fields, which will be passed to Emacs. The keys should be keywords
   and values functions. For windows and groups the function takes a
   single argument, while frame functions should take both the group
   and the frame (in that order).
...
*** Marking
    ...
    ~stumpbuffer-mark~ and ~stumpbuffer-unmark~ can be used to add and
...
*** Custom mark functions
    ...
    mark characters and function to call during
...
*** Filters
    ...
    function should take a single argument, a group or window plist,
    and return ~t~ if it should be filtered out.
