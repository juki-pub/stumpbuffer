...
** Filters
   ...
   The variable ~stumpbuffer-filter-groups~ contains a list of filter
   groups. Each group should be a list of filters in form ~(what
   . how)~. ~what~ should be either ~:hide-groups~, ~:show-groups~,
   ~:hide-windows~ or ~:show-windows~. The ~:hide-~ variants hide
   matching windows or groups, while the ~:show-~ variants hide
   non-matching ones.
